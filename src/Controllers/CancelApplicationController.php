...
class CancelApplicationController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        try {
            $application = $this->applications->transition($application, 'cancelled', (int) $actor->id, trim((string) ($body['review_note'] ?? '')) ?: null);
        } catch (InvalidArgumentException $error) {
            return new JsonResponse(['error' => $error->getMessage()], 409);
        }

        $this->audit->record((int) $actor->id, 'application.cancelled', 'application', (string) $application->id, $before, ['status' => 'cancelled'], $request);
